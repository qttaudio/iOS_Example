...
-(void) initData
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (outSourceEnabled) {
            self->mAudioController = [AudioController audioController];
...
- (void)onJoinSuccess:(NSString*)channelName uid:(NSUInteger)uid role:(QttChannelRole)role muted:(bool)muted isReconnect:(bool)isReconnect {
    ...
    if(!isReconnect){
        ...
        if (outSourceEnabled && self->mAudioController != nil) {
            [self->mAudioController startWork];
            [WHToast showMessage:@"采用的是外部音源通话" duration:2 finishHandler:^{
            }];
        }
    }else{
...
- (void)audioController:(AudioController *)controller didCaptureData:(unsigned char *)data bytesLength:(int)bytesLength {
    [rtcEngine pushExternalRecordFrame:(char*)data bufSize:bytesLength];
}

- (int)audioController:(AudioController *)controller didRenderData:(unsigned char *)data bytesLength:(int)bytesLength {
    std::lock_guard<std::mutex> lck(mMtx);
    if (mBuffer.size() > bytesLength) {
        memcpy(data, mBuffer.data(), bytesLength);
        mBuffer.erase(mBuffer.begin(), mBuffer.begin() + bytesLength);
        return bytesLength;
    }
